...
def optimize_tasks_with_gurobi():
    ...
    if model.status == GRB.OPTIMAL:
        ...
        def calculate_cost_for_intervals(task_rows, shift_row, weight, interval_minutes=15):
            ...
            ensuring tasks are not assigned during shift breaks and minimizing the maximum nurses required.
            """
            shift_start = pd.to_datetime(shift_row["StartTime"], format="%H:%M:%S")
            shift_end = pd.to_datetime(shift_row["EndTime"], format="%H:%M:%S")
            shift_breaks = [
                (pd.to_datetime(start, format="%H:%M:%S"), pd.to_datetime(start, format="%H:%M:%S") + pd.to_timedelta(duration, unit='m'))
                for start, duration in shift_row.get("Breaks", [])
            ]
            ...
            time_slots = {
                ...
                if all(not (start.timestamp() // 60 <= minute < (start + pd.to_timedelta(duration, unit='m')).timestamp() // 60)
                    for start, duration in shift_row.get("Breaks", []))
            }
            
            assignments = []

            # Assign tasks to minimize the overall cost
            for task_row in task_rows:
                ...
                start_times = []
                for start in pd.date_range(start=start_time, end=end_time - pd.Timedelta(minutes=duration_minutes), freq=f"{interval_minutes}min"):
                    end = start + pd.Timedelta(minutes=duration_minutes)
                    if all(not (break_start <= start < break_end or break_start < end <= break_end) for break_start, break_end in shift_breaks):
...
def new_optimize(tasks, shifts):
    ...
    tasks (list): List of tasks with attributes:
                  - name: Task name
                  - start_window: Start time window (in minutes from midnight)
                  - end_window: End time window (in minutes from midnight)
                  - duration: Task duration (in minutes)
                  - nurses_required: Number of nurses required
    shifts (list): List of shifts with attributes:
            ...
            with opt_tab:
                ...
                if st.button("🚀 Run Task Optimization (Original)", use_container_width=True):
                    ...
                    c.execute('''
                        ...
                        VALUES
                        ('Task 1', 'Monday', '09:00:00', '22:00:00', 30, 5),
                        ('Task 2', 'Monday', '09:00:00', '22:00:00', 120, 7),
                        ('Task 3', 'Monday', '09:00:00', '22:00:00', 30, 1),
                        ('Task 4', 'Monday', '09:00:00', '22:00:00', 30, 6),
                        ('Task 5', 'Monday', '09:00:00', '22:00:00', 30, 3),
                        ('Task 6', 'Monday', '09:00:00', '22:00:00', 30, 2),
                        ('Task 7', 'Monday', '09:00:00', '22:00:00', 30, 2)
                        ''')
                    ...
                    c.execute('''
                        ...
                        VALUES
                        ('07:00:00', '23:00:00', '08:30:00', '0:30:00', 10, 1, 1, 0, 0, 1, 0, 1), 
                        ('08:00:00', '23:00:00', '08:30:00', '0:30:00', 20, 1, 1, 0, 0, 1, 0, 1)
                        ''')
                    conn.commit()
                    conn.close()
